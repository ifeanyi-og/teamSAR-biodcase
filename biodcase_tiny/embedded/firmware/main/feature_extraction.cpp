...
esp_err_t extract_features(
    ...
    constexpr int N = 26;  // Number of FFT frames
    constexpr int M = 512; // FFT size (interleaved real/imag)
    std::array<float, N> sqrt_flux;

    // 1. Allocate storage
    std::array<std::array<int16_t, M>, N> fft_frames;
    std::array<int32_t, N> flux_values;  // Flux per frame

    // 2. Generate random FFTs
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
            fft_frames[i][j] = rand() % 256 - 128;  // simulate real/imag [-128, 127]
        }
    }

    auto handle = profiler->BeginEvent("flux");
    for (int i = 1; i < N; ++i) {
        int32_t flux = 0;

        for (int j = 0; j < M; ++j) {
            int16_t diff = fft_frames[i][j] - fft_frames[i - 1][j];
            flux += diff * diff;
        }

        flux_values[i] = flux;
        sqrt_flux[i] = int_sqrt(flux);
    }
    flux_values[0] = 0;
    sqrt_flux[0] = 0;
    ...
    float flux_stddev = 0.0f;

    handle = profiler->BeginEvent("mean_stddev");
    ...
    flux_mean = static_cast<float>(sum) / N;

    //stddev
    float sq_sum = 0.0f;
    for (int i = 0; i < N; ++i) {
        float diff = static_cast<float>(sqrt_flux[i]) - flux_mean;
        sq_sum += diff * diff;
    }
    flux_stddev = std::sqrt(sq_sum / N);
