...
def make_tf_datasets(
    ...
    for split, group_data in data.groupby("split"):
        ...
        #autocorr = np.array(group_data["autocorr"].to_list())
        # one_hot_labels = to_categorical(group_data["label"], num_classes=2)
        one_hot_labels = to_categorical(group_data["label"], num_classes=2)
        dataset = tf.data.Dataset.from_tensor_slices((features, one_hot_labels))
        # dataset = tf.data.Dataset.from_tensor_slices(((features, autocorr), one_hot_labels))
        dataset = dataset.shuffle(buffer_size=buffer_size, seed=seed).batch(batch_size)
        splits[split] = dataset

    # print("Unique labels for split:", split, np.unique(one_hot_labels))
    # print("Sample label shape:", one_hot_labels.shape)
...
def run_model_training(config: Config):
    ...
    # autocorr_shape = (features_shape[0], )
    ...
    for i, row in data.head(50).iterrows():
        ...
        print("Input vector:", row["features"])
        print("---")



    X = np.stack(data["features"].values)  # shape: (num_samples, 4)
    ...
    # data["label"] = data["label"].apply(lambda x : ((2 * x) - 1))       # SVM dataset: ignore
    ...
    model = create_model(features_shape)
    # model = create_model()
    # model = create_model((*features_shape, 1), autocorr_shape)
    #'''
