...
from biodcase_tiny.feature_extraction.feature_extraction import process_window, make_constants, do_autocorr
...
def plot_features_sample(sample: pd.DataFrame, features_shape):
    ...
    for i, (idx, row) in enumerate(sample.iterrows()):
        path, audio_data, sample_rate = row["path"], row["autocorr"], row["sample_rate"]
        fig.add_trace(
            go.Heatmap(
...
def apply_windowed(data, window_len, window_stride, fn):
    ...
    for row in v:
        ...
        # print(np.max(outs))
        #'''
    ## then add the autocorrelation as bottom row
    # foo = autocorr(data, window_len, window_stride)
    # print(np.max(foo))
    # outs.append(foo)
    if (False):
        print("max outs = ", np.max(outs))
        print("min outs = ", np.min(outs))
        print("size outs = ", np.size(outs))
        print("foo = ", foo)
        print("size autocorr", np.size(foo)) # '''
...
def run_feature_extraction(config: Config):
    ...
    with TqdmCallback(desc="Extracting features from preprocessed data"):
        data = data.map_partitions(
            lambda df: df.assign(
                features = df["data"].apply(
                    lambda clip: do_windows_fn(clip).flatten()
                    ),
                autocorr = df["data"].apply(
                    lambda clip: do_autocorr(clip, fe_config.window_len, fe_config.window_stride).flatten())

                # lambda clip: do_windows_fn(clip))
                # .apply(lambda f: extract_dct_from_features(f, m, n))
                # add/remove the 2d convolution
            ),
            meta=pd.DataFrame(
                dict(
                    **{c: data._meta[c] for c in data._meta},
                    features=pd.Series([], dtype=object),
                    autocorr=pd.Series([], dtype=object),
                )
            )
        )
        sample = data.head(10)      # first 10 samples that go to be plotted
        print(data)
        features_tshape = (m, n)
