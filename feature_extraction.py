...
from biodcase_tiny.feature_extraction.feature_extraction import process_window, make_constants, gfcc1_features, gfcc2_features
...
def apply_windowed(data, window_len, window_stride, fn):
    ...
    outs = np.array(outs)
    autocorr, periodogram = gfcc1_features(outs[:,0])
    zero_crossr, form_factor = gfcc2_features(outs[:, 1])

    prndl = np.array([autocorr, periodogram, zero_crossr, form_factor]).reshape(4, 1)
    #print(prndl)
    return prndl
...
if __name__ == "__main__":
    ...
    # b = pd.read_parquet('data\\03_features\\features.parquet\\part.3.parquet', engine='pyarrow')
    # print(b(1))
