...
def apply_windowed(data, window_len, window_stride, fn):
    ...
    for row in v:
        # euclidian distance between each fft, with rach fft band being a dimension
        
        fft_curr = fn(row)
        d = fft_curr - fft_prev
        fft_prev = fft_curr
        flux_n = np.sqrt(np.sum(np.power(d, 2)))
        flux.append(flux_n)

    flux_mean = np.mean(flux)
    flux_std = np.std(flux)

    return np.array([flux_mean, flux_std]).reshape(2,1)
